...
void Camera::set_view(Vector3 view_at, Vector3 view_from)
{
	view_at_ = view_at;
	view_from_ = view_from;
	Update();
...
void Camera::set_view_at(Vector3 view_at)
{
	view_at_ = view_at;
	Update();
}

void Camera::set_view_from(Vector3 view_from)
{
	view_from_ = view_from;
	Update();
}
...
{
	const float dx = pixel_size_ * (sx - 0.5f * (width_ - 1)); // direction x coord
	const float dy = pixel_size_ * (-sy + 0.5f * (height_ - 1)); // direction y coord

	Vector3 direction = Vector3(dx, dy, -d_); // direction of the new ray with new x, y and negative focal length
	direction.Normalize();

	direction = view_t_ * direction; // eye space -> world space
	direction.Normalize();


	return Ray(view_from_, direction, 0); // return new ray from the view point, in computed direction
}
